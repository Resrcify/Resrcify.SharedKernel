...
public class Result
{
        ...
        params (Func<T, bool> predicate, Error error)[]? functions)
    {
        ...
        if (functions.Length == 0)
            return Failure<T>(Error.EmptyInput);

        List<Error>? failures = null;
        HashSet<Error>? seen = null;
        ...
        foreach (var (predicate, error) in functions)
        {
            ...
            AddUniqueError(error, ref failures, ref seen);
        }

        return failures is null
            ? Success(value)
            : Failure<T>(CreateErrorsArray(failures));
    }
    ...
    public static Result<TValue> Combine<TValue>(
        Func<TValue>? successValueFactory,
        params Result[]? results)
    {
        if (successValueFactory is null || results is null)
            return Failure<TValue>(Error.NullValue);

        if (results.Length == 0)
            return Failure<TValue>(Error.EmptyInput);

        var failures = CollectFailures(results);

        return failures.Length > 0
            ? Failure<TValue>(failures)
            : CreateSuccessFromFactory(successValueFactory);
    }
    ...
    private static Error[] CollectFailures(params Result[]? results)
    {
        ...
        return failures is null
            ? EmptyErrors
            : CreateErrorsArray(failures);
    }

    private static void AddUniqueError(
        Error error,
        ref List<Error>? failures,
        ref HashSet<Error>? seen)
    {
        if (error == Error.None)
            return;
    ...
    private static Error[] CreateErrorsArray(List<Error> errors)
        => [.. errors];

    private static Result<TValue> CreateSuccessFromFactory<TValue>(Func<TValue>? successValueFactory)
        => successValueFactory is null
            ? Success(default(TValue)!)
            : Success(successValueFactory());
}
