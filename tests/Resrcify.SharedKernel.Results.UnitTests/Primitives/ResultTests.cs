...
public sealed class ResultTests
{
    ...
    [Fact]
    public void Ensure_WithEmptyFunctions_ShouldReturnFailureResult()
    {
        int value = 42;
        var result = Result.Ensure(value, Array.Empty<(Func<int, bool> predicate, Error error)>());

        result.IsFailure.ShouldBeTrue();
        result.Errors.ShouldContain(Error.EmptyInput);
    }

    [Fact]
    public void Ensure_WithNullFunctions_ShouldReturnFailureResult()
    {
        int value = 42;
        (Func<int, bool> predicate, Error error)[]? functions = null;

        var result = Result.Ensure(value, functions!);

        result.IsFailure.ShouldBeTrue();
        result.Errors.ShouldContain(Error.NullValue);
    }
    ...
    [Fact]
    public void Combine_Generic_WithEmptyResults_ShouldReturnFailureResult()
    {
        var combinedResult = Result.Combine<int>();

        combinedResult.IsFailure.ShouldBeTrue();
        combinedResult.Errors.ShouldContain(Error.EmptyInput);
    }

    [Fact]
    public void Combine_Generic_WithNullResults_ShouldReturnFailureResult()
    {
    ...
    [Fact]
    public void Combine_WithFactory_AndEmptyResults_ShouldReturnFailureResult()
    {
        var combinedResult = Result.Combine(() => 99);
    ...
    [Fact]
    public void Combine_WithNullFactory_AndEmptyResults_ShouldReturnFailureResult()
    {
        Func<int>? factory = null;

        var combinedResult = Result.Combine(factory!);
